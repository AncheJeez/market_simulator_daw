...
export default function TradingSocialFeed({ type, limit = 3 }: FeedProps) {
        ...
        const fetchData = async () => {
            try {
                if (type === 'news') {
                    ...
                    const res = await fetch(`https://www.reddit.com/r/trading+DayTrading/top.json?limit=${limit}&t=day`);
                    const data = await res.json();
        ...
        <div className="card border-0 shadow-lg mb-4" style={{ 
            ...
            <div className="card-header bg-transparent border-bottom border-secondary p-3">
                <h6 className="mb-0 text-uppercase fw-bold" style={{ 
                    ...
                    {isReddit ? 'Reddit Community Top' : 'Global Market News'}
                </h6>
            </div>
